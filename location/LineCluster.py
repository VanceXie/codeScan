...
def cluster_lines(lines: list, eps: int) -> dict:
	...
	# 使用DBSCAN进行聚类
	db = DBSCAN(eps=eps, min_samples=10, metric='manhattan')
	labels = db.fit_predict(lines)
...
def line_detect(lsd: cv2.LineSegmentDetector, edges: np.ndarray):
	...
	:return: Dictionary with slope as key and ndarray constructed by end point coordinates of the line segment as value
	"""
	# detect lines
...
@calculate_time
def find_barcode_by_cluster(image: np.ndarray, lsd: cv2.LineSegmentDetector, eps: int) -> dict:
	"""
	:param image: image as np.ndarry, gray or signal channel
	:param lsd: object of LineSegmentDetector
