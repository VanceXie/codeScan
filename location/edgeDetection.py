...
def nothing(x):
    ...
    edge_horizon = cv2.convertScaleAbs(edge_horizon)
    edge_vertical = cv2.Sobel(gray, cv2.CV_64F, 0, 1, ksize=int(2 * x + 1))
    edge_vertical = cv2.convertScaleAbs(edge_vertical)
    edge = edge_horizon - edge_vertical
    edge = edge
    edge = cv2.normalize(edge, None, 0, 255, cv2.NORM_MINMAX, cv2.CV_8UC1)
...
cv2.createTrackbar('R', "Barcode Detection", 3, 15, nothing)
nothing(3)
if cv2.waitKey(0) == 27:
