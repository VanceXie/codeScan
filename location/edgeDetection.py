...
def nothing(x):
    ...
    edge_gray = cv2.medianBlur(edge_show, 5)
    # 阈值处理
    ...
    # 分割行
    a = result.shape[0] // 300
    rows = np.array_split(result, threshold.shape[0] // 300)
    ...
    result = cv2.vconcat([edge_horizon_show, edge_vertical_show, image])
    # 显示图像
