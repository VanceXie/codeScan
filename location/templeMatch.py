...
@PerformanceEval.calculate_time
def template_match_multi_new(img, template, angle_step=30, scale_step=0.2, similarity_threshold=1, overlap_threshold=0.3):
	# 获取模板图像的大小
	th, tw = template.shape[:2]
	# 初始化最大匹配度和对应的位置
	matches = []
	...
	# 取出得分最高的检测框
	max_loc = matches[0][0]
	cv2.rectangle(img, (max_loc[1], max_loc[0]), (max_loc[1] + tw, max_loc[0] + th), (0, 0, 255), 2)
...
# 读取目标图像和模板图像
target_img = cv2.imread(r"D:\fy.xie\fenx\fenx - General\Ubei\Test_Label1\4.tif")
...
img = template_match_multi_new(target_img, template_img, 180, 0.2, 0.99, 0.1)
